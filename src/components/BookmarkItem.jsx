...
export default function BookmarkItem({
  ...
  return (
    <div
      className="bookmark--card relative p-6 rounded-2xl 
             w-xs sm:w-sm md:w-lg max-w-xl min-h-80"
    ...
    >
      <div className="flex justify-between items-center mb-1 bookmark--header">
        ...
        <FontAwesomeIcon
          icon={faTrashCan}
          className="text-[var(--text-2)] hover:text-red-600 text-lg md:text-xl active:scale-95 duration-150 cursor-pointer"
          onClick={(e) => {
            e.stopPropagation()
      ...
      {/* dia de mês de ano */}
      <span className="bottom-6 absolute text-sm md:text-base" >{date}</span>
      {/* </div> */}
    </div>
  )
}
